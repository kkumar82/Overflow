...
app.MapPost("/votes", async (CastVoteDto dto, VoteDbContext db, ClaimsPrincipal user,
    IMessageBus bus) =>
{
    ...
    var alreadyVoted = await db.Votes.AnyAsync(x =>
        x.TargetId == dto.TargetId && x.UserId == userId);
    ...
    var reason = (dto.VoteValue, dto.TargetType) switch
    {
        ...
        (-1, "Question") => ReputationReason.AnswerDownvoted,
        _ => ReputationReason.QuestionDownvoted
    };

    await bus.PublishAsync(
        ReputationHelper.MakeEvent(dto.TargetUserId, reason, userId));
