...
import {
    differenceInCalendarDays, differenceInCalendarMonths, differenceInCalendarWeeks,
    formatDistanceToNow, isToday, isYesterday
} from "date-fns";
...
export function fuzzyTimeAgo(date: string | Date) {
    ...
    if (weeks < 4) return `${weeks} day${weeks > 1 ? 's' : ''} ago`;
